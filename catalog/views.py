...
def home(request):
    return render(request, 'catalog/home.html', {'featured': Product.objects.filter(is_active=True)[:4], 'categories': Category.objects.all()[:4]})
def shop(request):
    query = request.GET.get('q', '').strip()
    category = request.GET.get('category', '')
    products = Product.objects.filter(is_active=True).select_related('category','seller')
    if query: products = products.filter(Q(name__icontains=query) | Q(category__name__icontains=query) | Q(city__icontains=query))
    if category: products = products.filter(category__slug=category)
    return render(request, 'catalog/shop.html', {'products': products, 'categories': Category.objects.all(), 'query':query, 'current_category':category})
